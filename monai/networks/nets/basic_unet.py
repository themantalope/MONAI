...
class BasicUNetBW(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x3 = self.down_3(x2)
        self.intermediate['x3'] = x3

        x4 = self.down_4(x3)
        self.intermediate['x4'] = x4

        u4 = self.upcat_4(x4, x3)
        self.intermediate['u4'] = u4
        
        u3 = self.upcat_3(u4, x2)
        self.intermediate['u3'] = u3
        
        u2 = self.upcat_2(u3, x1)
        self.intermediate['u2'] = u2
        
        u1 = self.upcat_1(u2, x0)
        self.intermediate['u1'] = u1

        logits = self.final_conv(u1)
        
        if self.return_intermediate:
            return logits, self.intermediate
        else:
            return logits
